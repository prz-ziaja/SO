...
int main(void)
{
  ...
  /* setting new file size limit */
  newlimit.rlim_cur = 50;
  newlimit.rlim_max = RLIM_INFINITY;
  setrlimit(RLIMIT_FSIZE, &newlimit);
  printf("NEW RLIMIT SET\n");
...
int parsecmd(char* __buf, int __bufsize, struct cmdlist* __head)
{
  ...
  while((word = strtok(cmd, " \t\n")) != NULL){
	...
	{
		struct cmdlist *newcmd = malloc(sizeof(struct cmdlist));
		setupnewcommand(newcmd);
	...
	else
	{
	    ...
	    curr->argv[curr->argc-1] = word;
                                      ...
                                      /* Storing new argument in the argument vector in our structure */
    cmd = NULL;
  }
...
int executecmds(struct cmdlist* __head)
{
  ...
  while(curr != NULL){
    ...
    if((curr->conjuction == CONJOR && wstatus == 0) || (curr->conjuction == CONJAND && wstatus != 0))
    {
	curr=curr->next;
	continue;
    }
    ...
    if(f == 0){/*wykona sie tylko w dziecku*/
      execvp(curr->argv[0], curr->argv);

      /*wykona sie tylko w przypadku bledu execvp*/
      e = errno;
