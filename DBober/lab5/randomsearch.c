...
void *search(void *arg){
	struct thread_params *tmp = arg;
	pthread_cleanup_push(cleanup,(void *)tmp);
     	int threadIdx = tmp->thread_idx; // indeks watku
     	int toFind = tmp->target;   // wartosc do odszukania
     	int ti = 0;
     	int rnd;
...
void cleanup(void *args){
	struct thread_params *tmp = args;
	printf("The thread no %d number of iterations: %d\n",tmp->thread_idx, tmp->num);
}
