...
int parsecmd(char* __buf, int __bufsize, struct cmdlist* __head)
{
  ...
  while((word = strtok(cmd, " \t\n")) != NULL){
	...
	if(strcmp(word, "&&") == 0 || strcmp(word, "||") == 0){
		...
		curr->next = tmp;
		if(setupparsedcommand(curr) == RESERROR) return RESERROR;
		curr = tmp;
		setupnewcommand(curr);
		curr->conjuction = (strcmp(word, "&&") == 0) ? CONJAND : CONJOR;	
	}
	else{
		curr->argc++;
...
int executecmds(struct cmdlist* __head)
{
  ...
  while(curr != NULL){
	...
	if (curr->argv[0] != NULL && (strcmp(curr->argv[0], "exit") == 0))
            exit(EXIT_SUCCESS);
	if(procres){
		if	(curr->conjuction == CONJAND && WEXITSTATUS(status) != 0) 	flag = 0;
		else if	(curr->conjuction == CONJOR && WEXITSTATUS(status) == 0)  	flag = 0;
	}
	if(flag){
	   	f = fork();
	    	e = errno;
