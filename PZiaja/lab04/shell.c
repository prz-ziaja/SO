...
int executecmds(struct cmdlist* __head)
{
  ...
  while(curr != NULL){
	...
	exit(EXIT_SUCCESS);
    if (procres) {
        if ((WEXITSTATUS(wfe) != 0)&&(curr->conjuction == CONJAND))
            ff = 0;
        else if ((WEXITSTATUS(wfe) == 0)&&(curr->conjuction == CONJOR))
            ff = 0;
    }
    if(ff){
	    f = fork();
	    ...
	    if (wait(&wfe) == -1) {//wait till child is dead
	      printf("Error: %s", strerror(e));
	      return RESERROR;
	    }
	    procres = WIFEXITED(wfe) ? 1 : 0;//the last words of child (returns true or false)
	    if (procres)
	      printf("%d \n", WEXITSTATUS(wfe));//if child said something bad
    }
    curr = curr->next;
    ff=1;
  }
