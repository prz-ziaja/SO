...
void ObslugaSIGINT(int i)
{
  printf("SIGINT: getpid()=%i, konto1+konto2=%li\n",
	 getpid(), //getpid() function shall return the process ID of the calling process//
    c->konto1+c->konto2
	 );
  koniec=1;
}
...
int main()
{
  signal(SIGINT,ObslugaSIGINT);   /* przechwycenie sygnału */


  int pid=fork(); //tworzymy nowy proces//
  if(pid!=0)
    { // pm
	...
	{ // pp2
	  ...
	  int id= shmget(KLUCZ, 0, 0);       //tworzymt wspólną pamięć//
	  if(id==-1) perror("pp2: shmget; Blad !!!");

	  c=(struct KONTA *)shmat(id, 0, 0);    //zwraca wskaźnik do zaalokowanego obszaru w pamięci lub -1 w przypadku wystąpienia błędu//
	...
	if(koniec) break;
      ...
      i=shmctl(id, IPC_RMID, NULL);   //IPC_RMID - usuwa segment pamięci.//
