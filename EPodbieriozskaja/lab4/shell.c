...
int parsecmd(char* __buf, int __bufsize, struct cmdlist* __head)
{
    ...
    struct cmdlist* oper = NULL;

    /* Reading next word - read strtok(3)  */
    while((word = strtok(cmd, " \t\n")) != NULL){
        if(strcmp(word, "&&")==0){
            oper =(struct cmdlist*) malloc(sizeof(struct cmdlist));
            curr->next=oper;
            if(setupparsedcommand(curr)==RESERROR){
                printf("Error while moving to the next command!");
                return RESERROR;
            };
            curr = oper;
...
int executecmds(struct cmdlist* __head)
{
    ...
    while(curr != NULL){
        if ((curr->argv[0] != NULL) && (strcmp(curr->argv[0], "exit") == 0))
            exit(EXIT_SUCCESS);
        //exec program according to operators || &&
        if ((curr->conjuction == CONJAND ) && (WEXITSTATUS(status) != 0)) { //child succes term &&
            flag=0;
        } else if ((curr->conjuction == CONJOR) && (WEXITSTATUS(status) == 0)) { //child term unsucces ||
            flag=0;
        }
        if (flag) {
            f = fork();
            ...
            if (waitpid(-1, &status, 0)==-1) {  //suspends execution of the calling pr until child change state.
                printf("Error: %s", strerror(e)); //
                return RESERROR;
            }
            if(WIFEXITED(status)){
                proces=0;    //child terminated=>go to exec next command
            }else{
                proces =1; //if child didn't terminate => continue loop
            }
            if(proces == 0){
                printf("Process finished with value: %d\n", WEXITSTATUS(status));
            }
        }
        curr = curr->next;
        flag=1;
